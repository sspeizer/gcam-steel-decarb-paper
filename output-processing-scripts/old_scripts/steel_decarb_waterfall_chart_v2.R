...
# as well as material efficiency production figures
...
fig_dir <- paste0(run_dir,"/figures_waterfall_v2")
...
for (selected_year in seq(2020, 2050, 5)){
  results <- c("Ref emissions", "Energy efficiency contribution",
               ...
               "High scrap use contribution", "H2 contribution", 
               "CCUS contribution", "1p5 emissions")
  for (i in regions_aggregated) {
    ...
    # (fraction of production from scrap in 1.5 - fraction of production from scrap in ref)*(total
    # production in 1.5) * (average emissions intensity of scrap in 1.5 - emissions intensity of other techs in ref)
    ...
    frac_scrap_ref <- scrap_REF / ref_production
    frac_scrap_1p5 <- scrap_1p5 / price_induced_production
    
    high_scrap_contribution <- (frac_scrap_1p5 - frac_scrap_ref) * price_induced_production * (scrap_emissions_intensity - other_tech_emissions_intensity)
    ...
    # H2-BF: (difference in emissions intensity between BF in ref and H2-BF in 1p5)*(H2-BF production in 1p5)
    H2_BF_prod_1p5 <- (ironsteel_production_tech %>% filter(region == selected_region & year == selected_year & scenario == "1p5" & 
                                                              technology == "BLASTFUR with hydrogen"))$value
    H2_BF_emissions_1p5 <- (CO2_emissions_tech %>% filter(region == selected_region & year == selected_year & scenario == "1p5" & 
                                                            technology == "BLASTFUR with hydrogen"))$value
    H2_BF_emissions_intensity_1p5 <- H2_BF_emissions_1p5 / H2_BF_prod_1p5
    BF_std_prod <- (ironsteel_production_tech %>% filter(region == selected_region & year == selected_year & scenario == "Ref" & 
                                                              technology == "BLASTFUR"))$value
    BF_std_emissions <- (CO2_emissions_tech %>% filter(region == selected_region & year == selected_year & scenario == "Ref" & 
                                                            technology == "BLASTFUR"))$value
    ...
    CCS_contribution <- sum(filter(CO2_squestration_subsector, sector == "iron and steel", region ==  selected_region, scenario == "1p5", year == selected_year)$value) * -1
...
for (i in regions_aggregated){
  waterfall_data <- data.frame(filter(results_final_edit,region==i,`Mitigation measure` != "1p5 emissions", year == 2050)) %>%
    # add color
  ...
  ggsave(paste0(fig_dir, "/waterfall_chart_values_", i, ".png"), height = 7, width = 8, units = "in")
}

# MAKE PLOT OF PRODUCTION AND CONTRIBUTIONS FROM MATERIAL EFFICIENCY AND PRICE INDUCED -------------
for (i in regions_aggregated) {
  ggplot(data=filter(ironsteel_production, region == i, year %in% c(2015:2050), scenario=="ref_MEF" | scenario=="Ref"|scenario=="1p5"),
